    ...
    try
        ...
        if ~isempty(warnMsg)
            ...
            if rssq(tform.T(3,1:2)) < shift_allowed
                trform_2_pfor{total_wab_queue} = flip(tform.T(3,1:2));
            else
                trform_2_pfor{total_wab_queue} = [nan nan];
            end
        end
...
for kk = 1:length(stitching_queue)
    image_1_max{kk} = zeros(tiling_info.z_pixel,stitching_queue(kk).res_i);
    image_2_max{kk} = zeros(tiling_info.z_pixel,stitching_queue(kk).res_i);
    
    for mm = 1:shifting_key_n
        total_wab_queue = sub2ind([length(stitching_queue), shifting_key_n],kk,mm);
        image_1_max{kk}(((mm-1).*z_truncate + 1):((mm-1).*z_truncate + z_truncate),:) = image_1_max_pfor{total_wab_queue};
        image_2_max{kk}(((mm-1).*z_truncate + 1):((mm-1).*z_truncate + z_truncate),:) = image_2_max_pfor{total_wab_queue};
    end
end
...
parfor kk = 1:length(stitching_queue)
    try
        tform = imregtform(image_1_max{kk},image_2_max{kk},'translation',optimizer,metric);
